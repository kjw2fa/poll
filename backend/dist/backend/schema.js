...
const resolvers = {
    ...
    Mutation: {
        createPoll: (parent, { title, options, userId }) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            yield (0, db_utils_1.dbRun)('INSERT INTO PollPermissions (pollId, permission_type, target_type, target_id) VALUES (?, ?, ?, ?)', [pollId, enums_1.PermissionType.EDIT, enums_1.TargetType.USER, parsedUserId]);
        ...
        editPoll: (parent, { pollId, userId, title, options }) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const { id: userIdStr } = fromGlobalId(userId);
            const parsedUserId = parseInt(userIdStr, 10);
            const permission = yield (0, db_utils_1.dbGet)('SELECT permission_type FROM PollPermissions WHERE pollId = ? AND target_id = ? AND permission_type = ?', [parsedPollId, parsedUserId, enums_1.PermissionType.EDIT]);
            if (!permission) {
    ...
    PollPermissions: {
        target_id: (parent) => {
            if (String(parent.target_type) === enums_1.TargetType.USER) {
                if (parent.target_id) {
    ...
    User: {
        ...
        polls: (parent, { permission }) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const userId = parseInt(userIdStr, 10);
            if (permission) {
                return yield (0, db_utils_1.dbAll)('SELECT Polls.* FROM Polls JOIN PollPermissions ON Polls.id = PollPermissions.pollId WHERE PollPermissions.target_id = ? AND PollPermissions.permission_type = ?', [userId, permission]);
            }
            return yield (0, db_utils_1.dbAll)('SELECT Polls.* FROM Polls JOIN PollPermissions ON Polls.id = PollPermissions.pollId WHERE PollPermissions.target_id = ?', [userId]);
        }),
        votes: (parent) => __awaiter(void 0, void 0, void 0, function* () {
            const { id: userIdStr } = fromGlobalId(parent.id);
            const userId = parseInt(userIdStr, 10);
            return yield (0, db_utils_1.dbAll)('SELECT * FROM Votes WHERE userId = ?', [userId]);
        })
    }
};
