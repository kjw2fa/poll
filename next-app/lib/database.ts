    ...
    const db = new sqlite3.Database(DBSOURCE, (err: Error | null) => {
        if (err) {
            ...
            db.serialize(() => {
                ...
                db.run(`CREATE TABLE IF NOT EXISTS Votes ( voteId INTEGER PRIMARY KEY AUTOINCREMENT, pollId INTEGER, userId INTEGER, UNIQUE(pollId, userId) )`);
                db.run(`CREATE TABLE IF NOT EXISTS VoteDetails ( id INTEGER PRIMARY KEY AUTOINCREMENT, voteId INTEGER, optionId INTEGER, rating INTEGER, FOREIGN KEY (voteId) REFERENCES Votes(voteId), FOREIGN KEY (optionId) REFERENCES PollOptions(id) )`);
